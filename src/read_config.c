...
int read_config(mystuff_t *mystuff)
{
    ...
    if (i == 0) {
        ...
        mystuff->threads_per_grid_max = 262144;
    } else if (i == 2) {
        mystuff->threads_per_grid_max = 524288;
    } else {
        mystuff->threads_per_grid_max = 1048576;
    }
    ...
    if (mystuff->verbosity >= 1) {
        if (i == 0) {
            logprintf(mystuff, "  PrintMode                 full\n");
        } else {
            logprintf(mystuff, "  PrintMode                 compact\n");
        }
    }
    mystuff->printmode = i;
    ...
    for (i = 0; i < 256; i++)
        mystuff->stats.progressheader[i] = 0;
    ...
    for (i = 0; i < 256; i++)
        mystuff->stats.progressformat[i] = 0;
